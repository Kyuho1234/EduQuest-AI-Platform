...
검증 기준:
1. factually_correct: 문제의 사실관계가 컨텍스트 내용과 일치하는지
2. answer_correct: 정답이 실제 정답인지
3. explanation_accurate: 해설이 정확한지
4. quality_score: 문제의 전반적인 품질 (0.0~1.0)"""
                ...
                try:
                    ...
                    for q in result.get("questions", []):
                        # 품질 점수 계산
                        llm_check = q.get("llm_check", {})
                        llm_score = llm_check.get("quality_score", 0.5)
                        semantic_score = q.get("semantic_similarity", 0.5)
                        
                        # 최종 신뢰도 점수 계산 (RAG 유사도 60%, LLM 검증 40% 반영)
                        reliability = (semantic_score * 0.6) + (llm_score * 0.4)
                        
                        # 필수 필드가 있는지 확인하고 완성된 문제 정보 구성
                        verified_question = {
                    ...
                    print(f"[ERROR] LLM 검증 중 오류: {str(e)}")
            
            # 검증 실패 시 RAG 결과만 반환
            return {"verified_questions": [{
                **q,
                "reliability_score": round(q.get("semantic_similarity", 0) * 0.6, 2),  # RAG 점수만 60% 반영
                "llm_verification": {
